...
def random_image_from_folder(folder_name):
    ...
    if not folder_id:
        print(f"[ERROR] Không tìm thấy folder '{folder_name}'")
        return None
    ...
    return drive.files().get_media(fileId=chosen["id"]).execute()
...
def download_image(url):
    ...
    if response.status_code == 200 and "text/html" in response.headers.get("Content-Type",""):
        ...
        if confirm:
            response = session.get(f"{url}&confirm={confirm.group(1)}", headers=headers, timeout=30)
...
def resolve_image(value):
    if not value:
        if FOLDER_MAP:
            ...
            print(f"[INFO] Ô trống, random folder: {folder_name}")
...
def upload_photo(image_bytes, published=False):
    ...
    res = requests.post(
        ...
        files={"source": ("image.jpg", image_bytes, "image/jpeg")}, timeout=30,
    )
    result = res.json()
    pid = result.get("id")
    if pid: print(f"[OK] Upload ảnh: {pid}")
    else:   print(f"[ERROR] Upload thất bại: {result}")
    return pid

def post_to_facebook(caption, row):
    image_values = []
    v1 = str(row.get("IMAGE_PATH","")).strip()
    if v1: image_values.append(v1)
    idx = 2
    while True:
    ...
    if not image_values:
        res = requests.post(f"https://graph.facebook.com/v19.0/{PAGE_ID}/feed",
                            data={"message": caption, "access_token": FB_TOKEN}, timeout=30)
        return res.json().get("id")
    photo_ids = [pid for val in image_values
                 if (pid := upload_photo(resolve_image(val), published=False))]
    if not photo_ids:
        res = requests.post(f"https://graph.facebook.com/v19.0/{PAGE_ID}/feed",
                            data={"message": caption, "access_token": FB_TOKEN}, timeout=30)
        return res.json().get("id")
    ...
    res = requests.post(f"https://graph.facebook.com/v19.0/{PAGE_ID}/feed", data=data, timeout=30)
    result = res.json()
    print(f"[INFO] Đăng bài: {result}")
    return result.get("id")

def add_comment(post_id, text, image_value):
    if not text and image_value is None: return
    data = {"access_token": FB_TOKEN}
    if text: data["message"] = text
    img = resolve_image(image_value)
    if img:
        res = requests.post(f"https://graph.facebook.com/v19.0/{post_id}/comments",
                            data=data, files={"source": ("image.jpg", img, "image/jpeg")}, timeout=30)
    else:
        res = requests.post(f"https://graph.facebook.com/v19.0/{post_id}/comments",
...
for i, row in enumerate(records):
    ...
    if status == "Test ngay": pass
    elif gio_dang != current_time or status != "Chưa làm": continue
    ...
    if post_id:
        for idx in range(1, 4):
            ...
            if c_text or c_img is not None:
        ...
        sheet.update_cell(row_num, headers.index("POSTED_AT") + 1, now.strftime("%Y-%m-%d %H:%M"))
        print(f"[OK] Đăng thành công! Post ID: {post_id}")
    else:
        print(f"[ERROR] Đăng thất bại dòng {row_num}")
